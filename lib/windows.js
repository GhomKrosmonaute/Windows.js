...
class Window {
    ...
    constructor( _x, _y, _width, _height, options={} ){
        ...
        this.cursorGrab = typeof options.grab == 'boolean' ? options.grab : typeof theme.defaultOptions.grab == 'boolean' ? theme.defaultOptions.grab : false
                ...
                for(const _key in template[key]){
                    ...
                    if(template[key][_key] && typeof template[key][_key] == 'object'){
                        for(const __key in template[key][_key]){
                            ...
                            if(template[key][_key][__key] && typeof template[key][_key][__key] == 'object'){
                                for(const ___key in template[key][_key][__key]){
                                    const ___element = createElement(___key)
                                    __element.appendChild(___element)
                                }
                            }else if(typeof template[key][_key][__key] == 'string'){
                                __element.innerHTML = options[template[key][_key][__key]] || theme.defaultOptions[template[key][_key][__key]] || ''
                            }
                            _element.appendChild(__element)
                        }
                    }else if(typeof template[key][_key] == 'string'){
                        _element.innerHTML = options[template[key][_key]] || theme.defaultOptions[template[key][_key]] || ''
                    }
                    element.appendChild(_element)
                }
            }else if(typeof template[key] == 'string'){
                element.innerHTML = options[template[key]] || theme.defaultOptions[template[key]] ||  ''
            }
        ...
        // styles
        if(theme.style.header && theme.style.header.height) 
        this.element.style.setProperty('--window-header-height',theme.style.header.height)
    ...
    startListeners(){
        this.interval = setInterval(()=>{
            if(this.resizing){
                ...
                this.get('header').style.cursor = cursor || (this.cursorGrab ? 'grab' : 'default')
            }
        },20)
    }
    ...
    onDrag(){
        ...
        this.drag = true
        this.get('header').style.cursor = this.cursorGrab ? 'grabbing' : 'default'
    }
    onDrop(){
        this.resizing = false
        if(this.drag){
        ...
        this.drag = false
        this.get('header').style.cursor = this.cursorGrab ? 'grab' : 'default'
        if(this.y < 0){
            this.fullscreen = false
            this.onFullscreen()
        }
    ...
    static addTheme( name, template, style, defaultOptions={} ){
        themes[name] = {
            template : template,
            style : style,
